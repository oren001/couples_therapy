...
export default function Partner1Page() {
  ...
  const sendAudioToAPI = async (audioBlob: Blob) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8000/partner/1/audio', formData, {
        headers: {
  ...
  const sendTextMessage = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:8000/partner/1/text', {
        text: userMessage,
        partner_id: 1
      });
  ...
  const approveMessageToTherapist = async (message: Message, index: number) => {
    ...
    try {
      const response = await axios.post('http://localhost:8000/partner/1/approve', {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      <header className="bg-blue-600 text-white p-4 flex items-center">
        ...
        <h1 className="text-xl font-bold">Partner 1 - Therapy Session</h1>
      </header>
